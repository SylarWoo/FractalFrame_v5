...
from .models import MmfV2Marker
from .signal_catalog import get_mmf_v2_signal_catalog

POINT_MOMENTUM_TYPES = {
    "MMF_V2_HIGH": "down",
    "MMF_V2_RESISTANCE": "down",
    "MMF_V2_LOW": "up",
    "MMF_V2_SUPPORT": "up",
}

BREAKOUT_MOMENTUM_TYPES = {
    "MMF_V2_RESISTANCE_UP_BREAK": ("up", {"MMF_V2_LOW", "MMF_V2_SUPPORT"}),
    "MMF_V2_SUPPORT_DOWN_BREAK": ("down", {"MMF_V2_HIGH", "MMF_V2_RESISTANCE"}),
}

CLOSE_MOMENTUM_TYPES = {
    "MMF_V2_SUPPORT_UP_BREAK": ("up", {"MMF_V2_LOW", "MMF_V2_SUPPORT"}),
    "MMF_V2_RESISTANCE_DOWN_BREAK": ("down", {"MMF_V2_HIGH", "MMF_V2_RESISTANCE"}),
}


def build_mmf_v2_signal_frame(
    features: pd.DataFrame,
    markers: list[MmfV2Marker],
    *,
    period_seconds: int | None = None,
) -> list[dict[str, Any]]:
    signal_types = [str(entry["catalogId"]) for entry in get_mmf_v2_signal_catalog()]
...
def build_mmf_v2_signal_payloads(
    features: pd.DataFrame,
    markers: list[MmfV2Marker],
    *,
    period_seconds: int | None = None,
...
def _signal_payload(marker: MmfV2Marker) -> dict[str, Any]:
    return {
...
def _attach_momentum(
    ...
    marker: MmfV2Marker,
    sorted_markers: list[MmfV2Marker],
    features: pd.DataFrame,
...
def _set_momentum(
    ...
    end_index: int,
    momentum: float | None,
    previous_marker: MmfV2Marker | None = None,
) -> None:
...
def _find_previous_marker(sorted_markers: list[MmfV2Marker], marker_index: int, previous_types: set[str]) -> MmfV2Marker | None:
