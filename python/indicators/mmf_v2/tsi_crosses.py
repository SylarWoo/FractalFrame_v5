...
def create_tsi_cross_markers(features: pd.DataFrame, settings: MmfV2Settings) -> list[MmfV2Marker]:
    ...
    if bool(getattr(settings, "show_tsi_dead_cross_point", False)) or bool(getattr(settings, "show_tsi_dead_cross_confirm_point", False)):
        markers.extend(_create_side_markers(
            ...
            cross_type="MMF_V2_TSI_DEAD_CROSS",
            confirm_type="MMF_V2_TSI_DEAD_CROSS_CONFIRM",
            price_column="high",
    ...
    if bool(getattr(settings, "show_tsi_golden_cross_point", False)) or bool(getattr(settings, "show_tsi_golden_cross_confirm_point", False)):
        markers.extend(_create_side_markers(
            ...
            cross_type="MMF_V2_TSI_GOLDEN_CROSS",
            confirm_type="MMF_V2_TSI_GOLDEN_CROSS_CONFIRM",
            price_column="low",
...
def _create_marker(features: pd.DataFrame, cross_index: int, marker_index: int, marker_type: str, price_column: str, reason: str) -> MmfV2Marker | None:
