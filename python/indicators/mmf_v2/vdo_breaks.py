...
def create_vdo_threshold_markers(features: pd.DataFrame, settings: MmfV2Settings) -> list[MmfV2Marker]:
...
def create_vdo_market_markers(features: pd.DataFrame, settings: MmfV2Settings) -> list[MmfV2Marker]:
    specs: list[tuple[bool, str, str, str, str]] = [
        (
...
def _vdo_market_cross_indexes(features: pd.DataFrame, column: str, marker_type: str) -> list[int]:
    ...
    for index in range(1, len(features)):
        ...
        if not (finite_number(previous_vdo) and finite_number(previous_ma) and finite_number(current_vdo) and finite_number(current_ma)):
            continue
        if marker_type == "MMF_V2_BULL_MARKET" and float(previous_vdo) < float(previous_ma) <= float(current_vdo):
            indexes.append(index)
        if marker_type == "MMF_V2_BEAR_MARKET" and float(previous_vdo) > float(previous_ma) >= float(current_vdo):
            indexes.append(index)
    return indexes
...
def _create_vdo_break_markers(
    ...
    for index in indexes:
        ...
        markers.append(create_mmf_v2_marker(
            type=marker_type,
            event_index=index,
