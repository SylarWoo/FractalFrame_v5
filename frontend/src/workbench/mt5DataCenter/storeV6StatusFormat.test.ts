...
import type { StoreV6CheckPayload, StoreV6PullJobPayload } from '../../services/mt5/mt5SymbolsApi'

describe('StoreV6 status formatting', () => {
  it('formats pull progress from structured job state', () => {
    const job: StoreV6PullJobPayload = {
      ok: true,
      jobId: 'job-1',
      symbol: 'XAUUSDm',
      mode: 'incremental',
  ...
  it('resolves local M1 rows from legacy and current status shapes', () => {
    const status: StoreV6CheckPayload = {
      ok: true,
      status: 'ok',
      symbol: 'XAUUSDm',
      directM1: null,
...
describe('StoreV6 status NUL handling', () => {
