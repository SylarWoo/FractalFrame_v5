import type { Mt5RealtimeTick, Mt5SymbolRow, StoreV6CheckPayload } from '../../services/mt5/mt5SymbolsApi'
...
export const storeV6StatusChangedEvent = workbenchEvents.storeV6StatusChanged
...
const storePanelPersistenceKeys = [
  ...
  storageKeys.importCenterStoreV6Status,
  storageKeys.importCenterStoreV6ListSymbols,
...
export type PersistedM1CheckResult = {
  checkedAt: string
  payload: StoreV6CheckPayload
}

export type PersistedStoreV6Status = {
  checkedAt: string
  payload: StoreV6CheckPayload
}
...
export function savePersistedM1CheckResult(symbol: string, payload: StoreV6CheckPayload, checkedAt: string, enabled = true) {
  if (!enabled) return
  if (!symbol) return
  const parsed = readJson<Record<string, PersistedM1CheckResult>>(storageKeys.importCenterM1CheckResults, {})
...
export function readPersistedStoreV6Status(symbol: string, enabled = true): PersistedStoreV6Status | null {
  if (!enabled) return null
  if (!symbol) return null
  const parsed = readJson<Record<string, PersistedStoreV6Status>>(storageKeys.importCenterStoreV6Status, {})
  const item = parsed?.[symbol]
  if (!item || typeof item !== 'object' || !item.payload) return null
  if (typeof item.checkedAt !== 'string') return null
  return item
...
export function savePersistedStoreV6Status(symbol: string, payload: StoreV6CheckPayload, checkedAt = new Date().toISOString(), enabled = true) {
  if (!enabled) return
  if (!symbol) return
  const parsed = readJson<Record<string, PersistedStoreV6Status>>(storageKeys.importCenterStoreV6Status, {})
  const written = writeJson(storageKeys.importCenterStoreV6Status, {
    ...(parsed && typeof parsed === 'object' ? parsed : {}),
    [symbol]: { checkedAt, payload },
  })
  if (written) dispatchWorkbenchEvent(storeV6StatusChangedEvent)
}

export function readStoreV6ListSymbols(enabled = true): string[] {
  if (!enabled) return []
  const parsed = readJson<unknown[]>(storageKeys.importCenterStoreV6ListSymbols, [])
  return Array.isArray(parsed) ? parsed.filter((item): item is string => typeof item === 'string') : []
}

export function saveStoreV6ListSymbols(symbols: string[], enabled = true) {
  if (!enabled) return
  writeJson(storageKeys.importCenterStoreV6ListSymbols, [...new Set(symbols)])
}
