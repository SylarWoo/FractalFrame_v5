...
export function normalizePeriodForUi(period: string) {
  const normalized = period.trim().toUpperCase()
  return normalized === 'MN1' ? 'MN' : normalized
}

export function periodToStoreTimeframe(period: string) {
  const normalized = normalizePeriodForUi(period)
  return normalized === 'MN' ? 'MN1' : normalized
}
...
export function resolveLocalM1Rows(status: StoreV6CheckPayload | null) {
...
export function resolveLocalM1LastTime(status: StoreV6CheckPayload | null) {
...
export function storeTableKeyForPeriod(period: string, rows: StoreTableRow[] = []) {
  const normalized = normalizePeriodForUi(period)
  const visibleRow = rows.find((row) => normalizePeriodForUi(row.period) === normalized)
...
export function periodFromStoreTableKey(key: string) {
  ...
  return normalizePeriodForUi(parts[parts.length - 1] || '')
}
...
export function formatStoreOperationLine(
  pullJob: StoreV6PullJobPayload | null,
