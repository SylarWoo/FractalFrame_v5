...
import { delay } from '../mt5DataCenter/storeV6StatusFormat'
...
export function waitStoreV6PullJobBySse(jobId: string, {
  activePullJobRef,
  pullEventSourceRef,
  setPullProgress,
}: PullWaiterOptions) {
  return waitJobBySse<StoreV6PullJobPayload>(jobId, {
    activeJobRef: activePullJobRef,
    eventSourceRef: pullEventSourceRef,
    setProgress: setPullProgress,
    createEventSource: createStoreV6PullEventSource,
    cancelledError: 'store_v6_pull_cancelled',
    failedError: 'store_v6_pull_failed',
    disconnectedError: 'store_v6_pull_sse_disconnected',
  })
}

export async function waitStoreV6PullJobByPolling(jobId: string, {
  activePullJobRef,
  setPullProgress,
}: PullWaiterOptions) {
  return waitJobByPolling<StoreV6PullJobPayload>(jobId, {
    activeJobRef: activePullJobRef,
    setProgress: setPullProgress,
    fetchJob: fetchStoreV6PullJob,
    cancelledError: 'store_v6_pull_cancelled',
  })
}

export function waitStoreV6AggregateJobBySse(jobId: string, {
  activeAggregateJobRef,
  aggregateEventSourceRef,
  setAggregateProgress,
}: AggregateWaiterOptions) {
  return waitJobBySse<StoreV6AggregateJobPayload>(jobId, {
    activeJobRef: activeAggregateJobRef,
    eventSourceRef: aggregateEventSourceRef,
    setProgress: setAggregateProgress,
    createEventSource: createStoreV6AggregateEventSource,
    cancelledError: 'store_v6_aggregate_cancelled',
    failedError: 'store_v6_aggregate_failed',
    disconnectedError: 'store_v6_aggregate_sse_disconnected',
  })
}

export async function waitStoreV6AggregateJobByPolling(jobId: string, {
  activeAggregateJobRef,
  setAggregateProgress,
}: AggregateWaiterOptions) {
  return waitJobByPolling<StoreV6AggregateJobPayload>(jobId, {
    activeJobRef: activeAggregateJobRef,
    setProgress: setAggregateProgress,
    fetchJob: fetchStoreV6AggregateJob,
    cancelledError: 'store_v6_aggregate_cancelled',
  })
}
