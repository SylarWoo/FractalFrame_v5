...
import { stickerOverlayName } from '../stickerOverlay'
...
const v2DrawingOverlayNames = new Set([...drawingOverlayNames, stickerOverlayName])
...
function applyCursor(root: HTMLElement, cursor: string) {
  const restore: CursorRestore[] = [{
    cursor: root.style.getPropertyValue('cursor'),
    element: root,
...
function eventHitsTwoPointEndpoint(chart: Chart, paneMain: HTMLElement, paneId: string, points: Array<{ dataIndex?: number; timestamp?: number; value?: number }>, event: MouseEvent | PointerEvent | null) {
...
function resolveHoveredOverlayInfo(chart: Chart, event: MouseEvent | PointerEvent | null) {
  ...
  if (!instanceName || !paneId || !v2DrawingOverlayNames.has(instanceName)) return null
  ...
  if (instanceName === stickerOverlayName && !hasFiniteValue(points[0])) return null
  const paneMain = chart.getDom(paneId, DomPosition.Main)
  ...
  const isTwoPointEndpointHandle = (instanceName === trendLineOverlayName || instanceName === rulerOverlayName || instanceName === fibRetracementOverlayName) && (
    ...
    eventHitsTwoPointEndpoint(chart, paneMain, paneId, points, event)
  )
  return {
...
export function installKLineChartDrawingMouseEventsV2({
  chart,
  isDragActive = () => false,
}: {
  chart: Chart
  ...
  const updateHoverCursor = () => {
    hoverTimer = 0
    if (activePressCursor || yAxisPressActive || isDragActive()) return
  ...
  const scheduleHoverCursorUpdate = (event?: MouseEvent | PointerEvent) => {
    ...
    if (isDragActive() || yAxisPressActive || (event && eventHitsYAxis(chart, event))) {
      clearHoverCursor()
      return
    }
    activeHoverCursor?.force()
  ...
  const handlePointerDown = (event: PointerEvent) => {
    ...
    activePressCursor?.restore()
    activePressCursor = hoverInfo?.paneMain === paneMain ? applyCursor(paneMain, hoverInfo.cursor) : null
    window.addEventListener('pointerup', finishPress, true)
    window.addEventListener('pointercancel', finishPress, true)
    window.addEventListener('blur', finishPress, true)
  }
  ...
  const handlePointerMove = (event: PointerEvent) => {
    lastPointerEvent = event
    if (isDragActive() || yAxisPressActive || eventHitsYAxis(chart, event)) {
      clearHoverCursor()
      return
    }
  ...
  return {
    ...
    refreshHoverCursor: () => scheduleHoverCursorUpdate(),
  }
}
