...
function readMacdSnapshotContext(input: unknown) {
  const context = input && typeof input === 'object' ? input as Partial<MacdIndicatorSettings> & { pageKey?: string; period?: string; runtimeOnly?: boolean; settingsHash?: string; symbol?: string } : {}
  return {
...
export function calculateMacdRowsForKLineChart(dataList: KLineData[], inputContext: unknown): MacdIndicatorRow[] {
  const context = readMacdSnapshotContext(inputContext)
  if (context.pageKey && context.symbol && context.period) {
    const rows = mapPageIndicatorSnapshotToDataList<MacdIndicatorRow>({
      dataList,
      indicator: 'macd',
  ...
  return calculateWithoutFuturePlaceholders(
    dataList,
    (realRows) => calculateTradingViewMacdRows(realRows, inputContext as Partial<MacdIndicatorSettings>),
  )
}
...
export function ensureTradingViewMacdIndicator() {
  ...
  registerIndicator<MacdIndicatorRow>({
    ...
    calc: (dataList, indicator) => calculateMacdRowsForKLineChart(dataList, indicator.calcParams[0]),
  })
}
