...
function readTsiSnapshotContext(input: unknown) {
  const context = input && typeof input === 'object' ? input as Partial<TsiIndicatorSettings> & { pageKey?: string; period?: string; runtimeOnly?: boolean; settingsHash?: string; symbol?: string } : {}
  return {
...
export function calculateTsiRowsForKLineChart(dataList: KLineData[], inputContext: unknown): TsiIndicatorRow[] {
  const context = readTsiSnapshotContext(inputContext)
  if (context.pageKey && context.symbol && context.period) {
    const rows = mapPageIndicatorSnapshotToDataList<TsiIndicatorRow>({
      dataList,
      indicator: 'tsi',
  ...
  return calculateWithoutFuturePlaceholders(
    dataList,
    (realRows) => calculateTradingViewTsiRows(realRows, inputContext as Partial<TsiIndicatorSettings>),
  )
}
...
export function ensureTradingViewTsiIndicator() {
  ...
  registerIndicator<TsiIndicatorRow>({
    ...
    calc: (dataList, indicator) => calculateTsiRowsForKLineChart(dataList, indicator.calcParams[0]),
  })
}
