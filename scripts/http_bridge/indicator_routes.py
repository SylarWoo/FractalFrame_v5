...
def handle_indicator_post(handler: Any, parsed: ParseResult, services: Any) -> bool:
    if parsed.path not in {"/api/indicators/v1/mmf/calculate", "/api/indicators/v2/mmf/calculate", "/api/indicators/v2/mmf/jobs/start", "/api/indicators/v1/vmi/calculate"}:
        return False
    try:
        ...
        if not isinstance(payload, dict):
            ...
            return True
        if parsed.path == "/api/indicators/v2/mmf/jobs/start":
